...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	case trainingType == "Ходьба":
		...
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\nВес: %.2f\nРост: %.2f\n", trainingType, duration, distance, speed, calories, weight, height)
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	meanSpeedSquared := math.Pow(meanSpeed(action, duration)*kmhInMsec, 2)
	return ((walkingCaloriesWeightMultiplier*weight + (meanSpeedSquared/height)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
